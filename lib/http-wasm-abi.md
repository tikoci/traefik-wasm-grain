...
> `HttpWasmAbi` modules exposes and provides low-level access the "navite"
> [http-wasm ABI](https://http-wasm.io/http-handler-abi/).  
> The rest of the document is directly from the spec.
...
> For Grain, these are implemented by the `HttpWasm` wrapper.
...
> **GRAIN IMPLEMENTATION NOTE**
> 
> Similarly, `WasmHttp` converts the ABI "snake case" into camelCase with a leading underscore.
> So as Grain "unsafe provide" from host (Traefik), the binding looks like:
> ```grain
...
> ```
> which is mapped by `WasmHttp` as:
> ```
> @unsafe
...
> }
> ```
> While the provide is `@unsafe`, normal Grain types can be used when
> implementing a plugin as `WasmHttp` abstracts this module (`WasmHttpAbi`) foreign provides
> from host (Traefik)
...
> **GRAIN IMPLEMENTATION NOTE**
>
> Several constants include `feature_buffer_request`, `feature_buffer_response`, and `feature_trailers` 
> are not provided by Traefik, and will error.  As such, these are not imported them. 
>
> For example, this does not work:
> ```grain
> provide foreign wasm feature_buffer_request: WasmI32 as _featureBufferRequest from "http_handler"
> provide foreign wasm feature_buffer_response: WasmI32 as _featureBufferResponse from "http_handler"
> provide foreign wasm feature_trailers: WasmI32 as _featureTrailers from "http_handler"
> ```
...
> **GRAIN IMPLEMENTATION NOTE**
>
> `HttpWasm` does not provide a high-level helper, since "Body Functions" are
> also not supported.
>
...
> **GRAIN IMPLEMENTATION NOTE**
>
> Various constants `$log_level_debug` etc shown in docs are not exported by Traefik.
> e.g.
> ```grain
> provide foreign wasm log_level_debug: WasmI32 as _log_level_debug from "http_handler"
> provide foreign wasm log_level_info: WasmI32 as _log_level_info from "http_handler"
> provide foreign wasm log_level_warn: WasmI32 as _log_level_warn from "http_handler"
> provide foreign wasm log_level_error: WasmI32 as _log_level_error from "http_handler"
> provide foreign wasm log_level_none: WasmI32 as _log_level_none from "http_handler"
> ```
...
> **GRAIN IMPLEMENTATION NOTE**
>
> Various constants `$header_kind_request` etc shown in docs are not exported by Traefik.
>
> For example, this does not work:
> ```grain
> provide foreign wasm header_kind_request: WasmI32 as _header_kind_request from "http_handler"
> provide foreign wasm header_kind_response: WasmI32 as _header_kind_response from "http_handler"
> provide foreign wasm header_kind_request_trailers: WasmI32 as _header_kind_request_trailers from "http_handler"
> provide foreign wasm header_kind_response_trailers: WasmI32 as _header_kind_response_trailers from "http_handler"
> ```
...
> `WasmHttp` Grain wrapper does not implement any of the "Body Functions". 
> Possible, but more complex memory management and feature flags/etc. needed
...
> **GRAIN IMPLEMENTATION NOTE**
> `body_kind` not implemented in Traefik, the following will cause plugin not to load
> ```grain
>   provide foreign wasm body_kind_request: WasmI32 as _body_kind_request from "http_handler"
>   provide foreign wasm body_kind_response: WasmI32 as _body_kind_response from "http_handler"
> ```
